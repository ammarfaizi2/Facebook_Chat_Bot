...
"lampu"=>2,
    ...
    return false;
		...
		$nd=explode(",",$needle);
		$sts=false;
		$wcheck=function($qx,$hy,$wi=false){$jl=0;
			if($wi===true){
				foreach($hy as $t){
					foreach($qx as $r){
						if($t==$r){
			...
			break;
				...
				case 'saklar':
					...
					if(preg_match("#[^0-4\,]#",$b[0]) OR !in_array($b[1],array("on","off"))){
						...
						$a->saklar($b[0],$b[1]);
						$a->get_image();
					$msg=array('img','lampu.jpg',"siap kang !\n".$a->get_status());
					}
					break;
					case 'lampu':
					if($this->_msg=="status"){
						$a=new saklar();
						$a->get_image();
					$msg=array('img','lampu.jpg',$a->get_status());
					break;
					}
			}
		}
